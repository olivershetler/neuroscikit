...
def runGLM(y, X, split=0.7, model='sklearn', link='Poisson', custom_intercept=True, regularized=False, l_split=None):
    ...
    X[X != X] = 0
    # .reshape((1,-1))
    trainX, trainY, testX, testY = split_endog_exog(y, X, p=split)


    if model == 'sklearn':
        res, res_data, inp = fit_glm_sklearn(trainX, trainY, testX, testY, custom_intercept)
    elif model == 'statsmodels':
        res, res_data, inp = fit_glm_statsmodel(trainX, trainY, testX, testY, regularized=regularized)
...
def fit_glm_statsmodel(trainX, trainY, testX, testY, regularized=False):
    ...
    poisson_model = sm.GLM(trainY, trainX.T, family=Poisson(log()))
