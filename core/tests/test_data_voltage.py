...
def test_ephys_series():
    ...
    ephys_series = collection.data['channel_0']

    data = ephys_series.data
    prev = len(data)
    ...
    assert len(ephys_series.data) == prev/2
...
def test_ephys_collection():
    ...
    assert len(collection.data.keys()) == collection.num_channels
